...
\begin{itemize}
\item \textbf{Release}: de software is volledig samengesteld uit pakketten die voldoende metadata bevatten om de verschillende bronnen te beschrijven waarvan het pakket afhangt.
\item \textbf{Installatie}: de software moet overgebracht worden naar de gebruiker en geconfigureerd worden in voorbereiding op de activatie.
\item \textbf{Activeren}: tijdens de activatie wordt de software-uitvoering opgestart of worden de nodige triggers geplaatst om de executie op het gepaste tijdstip op te starten.
\item \textbf{Deactiveren}: dit is het tegengestelde van activeren. Deze stap is nodig zodat een aanpassing of herconfiguratie uitgevoerd kan worden.
\item \textbf{Updaten}: dit is het proces waarin de software wordt aangepast. Deze stap wordt vaak geactiveerd door het uitbrengen van een nieuwe versie van de software.
\item \textbf{Deïnstallatie}: tijdens deze stap zal de geïnstalleerd software van het gebruikerssysteem gehaald worden.
\end{itemize}
...
Als deze eisen niet voldaan zijn, zal de software niet correct werken.
...
Met behulp van de model-driven aanpak is het mogelijk voor gebruikers om meerdere pipelines en releases over verschillende infrastructuren te coördineren op een eenvoudige manier.
...
Het automatiseringsplatform bestaat uit een drie-lagen architectuur, een webinterface en mogelijkheden om build en release analyses uit te voeren.
...
\item \textbf{Resource}: een resource wordt gedefinieerd als een agentmachine waar stappen in uitgevoerd kunnen worden.
\item \textbf{Procedures}: procedures en stappen worden gebruikt om taken in ElectricFlow te definiëren.
...
Bij een error moet een rollback uitgevoerd worden.
Dit is de taak van de scriptschrijver \citep{softwareDeployment}.
...
Om in de ATLAS-samenwerking (meer specifiek het LCG/EGEE project, LHC Computing Grid/Enabling Grids for E-sciencE \citep{bird2005lhc}) om te gaan met de grote hoeveelheid bronnen, is er een volledig automatisch installatiesysteem ontworpen. 
\citet{salvo2008atlas} beschrijven de architectuur van het ontworpen systeem.
...
Grid is een gedistribueerde computerinfrastructuur voor geavanceerde wetenschappen.
De focus ligt bij het delen van resources en innovatieve applicaties.\citep{foster2001anatomy}.
...
\begin{itemize}
\item \textbf{RAI module}: de Request An Installation module dient als webinterface voor het ontvangen van user-driven installatie-aanvragen.
...
Als deze actie wordt uitgevoerd, krijgt de gebruiker een mail.
...
Als de controle succesvol uitgevoerd wordt, kan het installatieproces beginnen.
...
Het ATLAS project gebruikt CMT \citep{cmt} als configuratiemanager.
Met behulp van een configuratiebestand weten verscheidene tools hoe ze een pakket moeten afhandelen.
...
Het enige dat hierover vermeld wordt is dat de exit-code van de job niet rechtstreeks gebruikt wordt.
...
Door geen agents te gebruiken, zal Ansible geen resources gebruiken zolang dat Ansible het systeem niet aan het gebruiken is \citep{ansible}.

\citep{ansible} beschrijft verder dat Ansible gebruik maakt van \emph{Playbooks} voor de organisatie van de IT omgevingen.
Playbooks zijn YAML (Een human-readable data serialisatie taal (Een superset van JSON)) definities van taken die beschrijven hoe een taak moet geautomatiseerd worden.
...
Tijdens de bespreking van het probleem in Sectie~\ref{sec:probleem} werd aangehaald dat een ``rampenplan'' voorzien moet worden.
...
\citet{huang1995software} definiëren software rejuvenation als volgt:
\begin{displayquote}
...
Hiernaast zal ieder huis op zijn minste een badkamer, woonkamer, slaapkamer en keuken hebben.
...
Een container is goedkoop waardoor het creëren en het verwijderen van containers een snelle operatie is.
...
\citet{deploymentMethods} halen drie methodes aan om software te deployen:
\begin{itemize}
...
Maar, zoals \citet{deploymentMethods} al aangeven, is deze methode niet flexibel.
...
Als geweten is welke bestanden aangepast of gecreëerd zijn, kan het proces nagebootst worden op andere nodes \citep{deploymentMethods}.
...
De laatste techniek die \citet{deploymentMethods} aanhalen, is package-based deployment.
Met behulp van een batch file, waarin alle nodige commando's aanwezig zijn, kan een installatiepakket gedeployed worden naar een target node.
...
\begin{itemize}
\item \textbf{Big-bang}: iedere gebruiker van de applicatie zal op eenzelfde moment overschakelen van de oude naar de nieuwe software. 
...
\item \textbf{Gefaseerd}: de nieuwe software zal bij een gefaseerde deployment enkel toegepast worden in specifiek geselecteerde projecten.
...
\begin{figure}[!ht]
\centering
\makebox[0pt]{\includegraphics[scale=0.7]{afbeelding/clientServerSimple.png}}
...
\begin{itemize}
\item \textbf{Assertions}: deze worden gebruikt om beperkingen die aan de gebruikerskant waar moeten zijn te beschrijven.
...
\item \textbf{Configuraties}: deze zorgt voor een beschrijving van de software die wordt uitgegeven.
...
Bij het wijzigen van een software release zal het release dock verschillende events afvuren.
...
Een naïeve aanpak, volgens \citet{carzaniga2001design}, is het gebruiken van één centrale server waar alle inschrijvingen worden bijgehouden, waar alle events toekomen, waar de bestemming van het event beslist wordt en waar het event wordt doorgestuurd naar de gepaste subscribers.
...
Het is belangrijk om stil te staan bij enkele ontwerpbeslissingen zodanig dat de service die in Figuur~\ref{fig:pubsubService} zichtbaar is, implementeerbaar is.
...
Met de toolset kunnen .msi installatiepakketten, .msm merge modules en .msp patches gecombineerd worden tot een Windows executable \citep{wixToolset}.
...
Op deze manier is het mogelijk om verscheidene softwarepakketten te combineren tot één geheel.
...
\citet{xmill} geven al aan dat XML niet een van de meest efficiënte dataformaten is, maar het verhoogt de flexibiliteit wel.
...
NuGet is de package manager voor het Microsoft development platform \citep{nuget}.

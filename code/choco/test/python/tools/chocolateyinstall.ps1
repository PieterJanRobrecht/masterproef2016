...
$packageName= 'python' # arbitrary name for the package, used in messages
$toolsDir   = "$(Split-Path -parent $MyInvocation.MyCommand.Definition)"
$url        = 'http://10.0.2.2/nsis/test/python-2.7.3.msi' # download url
$url64      = '' # 64bit URL here or remove - if installer contains both (very rare), use $url
$fileLocation = Join-Path $toolsDir 'python-2.7.3.msi'
...
$packageArgs = @{
  ...
  softwareName  = 'python*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique
  checksum      = ''
